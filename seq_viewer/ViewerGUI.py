...
https://stackoverflow.com/questions/3085696/adding-a-scrollbar-to-a-group-of-widgets-in-tkinter/3092341#3092341
    ...
    def __init__(self, frame, fig, width=16):
        ...
        # self.tk_cnv.grid(row=2, column=0, sticky="nsew")
        ...
        # v_scroll.grid(row=2, column=0, sticky="ns")
        ...
        # self.cnv_widget.config(height=1087)
        ...
        self.windows_item = \
            self.tk_cnv.create_window((100, 900), window=self.mpl_cnv_widget, anchor='e',
                                      tag='self.canvas')
    ...
    def __fill_canvas(self, event):
        ...
        # print(size, '({0},{1})'.format(self.tk_cnv.winfo_reqwidth(), self.tk_cnv.winfo_reqheight()))
        # print('({0},{1})'.format(self.frame.winfo_width(), self.frame.winfo_height()))
        # Enlarge the windows item to the canvas width
        canvas_width = event.width
        canvas_height = event.height
        # print(event.widget, self.frame, self.cnv_widget, self)
        print('self.tk_cnv.bbox:', self.tk_cnv.bbox("all"))
        print('self.frame.bbox:', self.frame.bbox("all"))
        print('self.bbox:', self.bbox("all"))
...
class StartPage(Frame):
    ...
    def __init__(self, parent, controller):
        ...
        self.checkbox_frame = LabelFrame(self.controller, text="Boards", relief="sunken")
        self.checkbox_frame.pack(side="top", anchor="nw", padx=2, pady=2, fill="x")
        ...
        # self.canvas_body = Scrollable(self.controller, self.plot_fig)
        self.mpl_canvas = self.canvas_body.mpl_cnv
        # self.canvas_window(self.canvas_frame, self.plot_fig)
        # self.canvas_window(self.controller, self.plot_fig)

        # Instance variable for the widgets controlling the animation
        ...
        # self.toolbar = NavTb2(self.mpl_canvas, self.controller)
        ...
        # self.animator.step_up_button(self.controller)
        # self.animator.stop_button(self.controller)
        # self.animator.step_dwn_button(self.controller)
    ...
    def canvas_setup(self):
        ...
        self.canvas_body.pack(side="top", anchor="nw", fill="both")
        # self.canvas_body.pack()
        self.toolbar.update()
